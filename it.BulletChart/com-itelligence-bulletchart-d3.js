...
//				valueArray[2] = ((valueArray[0]>valueArray[1]) ? valueArray[0] : valueArray[1]);
				...
				// create data array
				var dataPairs = { "title":label,"subtitle":sublabel,"ranges":[-1],"measures":[-1],"markers":[-1] }; //,"measures":{valueArray[0]},"markers":{valueArray[1]} };
...
/*		var data2=[
		...
		var svg = d3.select($element[0]).selectAll("svg")
		    ...
		    .enter().append("svg")
		      ...
		      .attr("class", "axis")
		      .attr("tickCount", disclosureTickCount(width))
...
//			log(divHeight);
		...
		function disclosureTickCount(width) {
				var ticks = Math.round(width/50);
